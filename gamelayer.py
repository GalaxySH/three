...
class GameLayer(Layer):
    ...
    def __init__(self, hud, scenario):
        ...
        self.bunker = actors.Bunker(*scenario.bunker_position, self)
    ...
    def game_loop(self, _):
        ...
        for turret in self.turrets:
            # add everything from iterator into a list
            list_of_targets = list(self.collman_enemies.iter_colliding(turret))

            # create variable to hold target
            min_health_target = None

            # loop over list
            for target in list_of_targets:

                # check if potential target has less health than current target
                if min_health_target is None or target.health < min_health_target.health:

                    # set as new target
                    min_health_target = target

            # original code that had poor targeting outcome
            # obj = next(self.collman_enemies.iter_colliding(turret), None)

            turret.collide(min_health_target)
